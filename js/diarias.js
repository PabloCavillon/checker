...
/*var buttons_ayuda = document.getElementsByName('ayuda');
buttons_ayuda.forEach(btn => {
	btn.addEventListener("click", function(e){
		e.preventDefault();
		nombre_tarea = btn.value;
		...
		window.location= url + "ayuda/mostrar/" + nombre_tarea;
	
	});
})*/

var buttons_automatica = document.getElementsByName('automatica');
buttons_automatica.forEach(btn_automatica => {
	btn_automatica.addEventListener('click', function(e){
		e.preventDefault();
		var aux = url.split('/');
		var str = "";
		...
		window.open(str + btn_automatica.value);
	});
});
...
function comentario(id){
	...
	(async () => {
		const { value: text } = await Swal.fire({
			title: '<p class="alerta"><i class="far fa-comments"> </i></p>',
			input: 'textarea',
